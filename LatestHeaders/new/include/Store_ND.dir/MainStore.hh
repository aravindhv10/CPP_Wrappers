...
class _MACRO_CLASS_NAME_ {
  ...
  public:
    inline TYPE_INT const GET_LOCATION (TYPE_INT const y, TYPE_INT const x) const {
      auto const bin = FLATTEN_MAP(y,x);
      return CUMULATIVE[bin]+COUNTS[bin];
    }

    inline TYPE_INT const GET_LOCATION (TYPE_FLOAT const y, TYPE_FLOAT const x) const {
      auto const bin = FLATTEN_MAP(y,x);
      return CUMULATIVE[bin]+COUNTS[bin];
    }
...
#define _MACRO_CLASS_NAME_ Main_Store
