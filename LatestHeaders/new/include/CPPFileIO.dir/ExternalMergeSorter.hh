...
#define _MACRO_CLASS_NAME_ ExternalMergeSorter
	...
	private:
		...
		using TYPE_SELF =
			_MACRO_CLASS_NAME_ <TYPE_ELEMENT,cleanup>
		;
	...
	///////////////////////////////////
		...
		RM (std::string const filename) {
			ExternalStarter<true>::GET("/bin/rm")
				("-vf")("--")(filename)
			; //
		}
		...
		RMDIR (std::string const filename) {
			ExternalStarter<true>::GET("/bin/rmdir")
				("-v")("--")(filename)
			;
		}
		...
		MergeRange () {
			...
			{
				ExternalStarter<true>::GET("/bin/mv")
					("-v")("--")
		...
		inline void
		SortFileSlave (size_t const index) {
