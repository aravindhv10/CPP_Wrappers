...
class _MACRO_CLASS_NAME_ {
  public:
    ...
    inline bool IsValid() const {
        return (!std::isnan(theta)) && (!std::isnan(phi)) && (0 <= theta) &&
               (theta <= M_PI) && (-M_PI <= phi) && (phi <= M_PI);
    }

    inline double distance(PolarPoint const &other) const {
    ...
    static inline TYPE_SELF AS_LAT_LONG(double const lt, double const lg) {
