...
namespace NewHEPHeaders {
    ...
    namespace VECTORS {
        ...
        public:
            ...
            ~sortorder () {}
            template <typename T> inline bool operator  () (lorentz4vector <T> & a, lorentz4vector <T> & b) {
                if (a>b) {return ret;}
                else {return !ret;}
            }
            template <typename T> inline bool operator  () (lorentz4vector <T> * a, lorentz4vector <T> * b) {
        ...
        template <typename T> void cleanup_vectors ( std::vector <lorentz4vector <T>> & thevectors) {
            std::vector <lorentz4vector <T>> tmp_thevectors;
            for (size_t i = 0; i < thevectors.size (); i++)
                if (!thevectors[i].cleared ()) { tmp_thevectors.push_back (thevectors[i]); }
                if (tmp_thevectors.size () > 0) {
                    thevectors.resize (tmp_thevectors.size ());
                    size_t n = thevectors.size () * sizeof (lorentz4vector <T>);
        ...
        public:
            typedef euclid3vector  <TR> vector3 ;
            typedef lorentz4vector <TR> vector4 ;
            vector3 dir [3] , SpinVector ;
            vector4 TOP     ;
            // { T2 , T3 } => { Theta2 , Theta3 }
            TR T2 , T3 ;
            TR E2 , E3 ;
