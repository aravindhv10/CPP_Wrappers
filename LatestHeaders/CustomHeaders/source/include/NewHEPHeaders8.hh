...
namespace NewHEPHeaders /* The main lorentz vectors part: */ {
    namespace VECTORS {
        ...
        public:
            ...
            inline bool isFinal () const { return (d1 == -1) && (d2 == -1); }
        ...
        public:
            typedef VECTORS::ParticleNode <TR,TI> TYPE_Element ;
            typedef GenParticles          <TR,TI> TYPE_Self    ;
        public:
            inline size_t FindPID    ( long   const PID) const {
                TYPE_Self const & SELF = this[0] ;
                size_t ret = 0 ;
                for(size_t i=0;(i<SELF.size())&&(ret==0);i++)if(SELF[i].id()==PID){ret=i;}
                return ret;
            }
            inline size_t FindPIDMod ( long   const PID) const {
                TYPE_Self const & SELF = this[0] ;
                size_t ret = 0 ;
            ...
            inline size_t Recurse    ( size_t const idx) const {
                if (idx > 0) {
                    TYPE_Self const & SELF = this[0] ;
                    int PID = SELF[idx].id ();
            ...
            template <typename T1> inline void ReadFromDelphes (T1 & inref) {
                ...
                for(size_t i=0;i<limit;i++){
                    ...
                    if(inref.Particle_Status[i]==1){D1=0;D2=0;}
