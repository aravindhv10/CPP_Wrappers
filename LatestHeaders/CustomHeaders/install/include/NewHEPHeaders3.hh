...
namespace NewHEPHeaders {
    ...
    namespace MainEventData {
        ...
        public:
            ...
            EventData() : jet_def_nrmjet(fastjet::antikt_algorithm,0.4)
            {QuickConstruct();}
...
/* Set charge and type: */  {
    ...
    namespace TopReconstruction {
        const double g2              = 8.0 * MASS::W * MASS::W * 1.1663788 / (sqrt(2.0)*100000.0) ;
        const double g4              = g2 * g2 ;
        const double gammaw          = 2.085 ;
        const double total_top_gamma = (759627.259035 + 759633.464482) / 2.0 ;

        class TopTester {
        private:
            double matrix2  , measure   , weight    ;
            double e2       , e3        ;
            double angle    , mte2      ;
            double mt       , mt2       ;
            double st2      , st3       ;
            ...
            vector3 Velocity , dir2p , dir3p                           ;
            vector4 Top , UnBoostedTop                                 ;
            vector4 Daughters [3] , UnBoostedDaughters [3] , direction ;

            inline bool prepare2 () {
                weight = 0                                                      ;
                e3     = UnBoostedDaughters[1][3]                               ;
                /* Debugging check: */ { goodvals = (e3>0)                    ; if (!goodvals) {printf("BAD: e3<0\n");return goodvals;} }
                st3    = UnBoostedDaughters[1].pt() / UnBoostedDaughters[1].p() ;
                /* Debugging check: */ { goodvals = (st3>=-1.0) && (1.0>=st3) ; if (!goodvals) {printf("BAD: |st3|>1\n");return goodvals;} }
                dir3p  = UnBoostedDaughters[1].xyz.dir()                        ;
                angle  = dir2p * dir3p                                          ;
                return goodvals                                                 ;
            }
            inline bool prepare ( vector4 a , vector4 b , vector4 c ) {
                weight = 0 ;
                /* Prepare Phase Space: */ {
                    Daughters          [0] = a                                                      ;
                    Daughters          [1] = b                                                      ;
                    Daughters          [2] = c                                                      ;
                    Top                    = Daughters[0] + Daughters[1] + Daughters[2]             ;
                    direction              = Top.dir()                                              ;
                    Velocity               = ( direction * Top[3] / Top.m() ).xyz                   ;
                }
                /* Unboost the vectors: */ {
                    UnBoostedDaughters [0] = Daughters [0].LorentzBoostGamma (Velocity)             ;
                    UnBoostedDaughters [1] = Daughters [1].LorentzBoostGamma (Velocity)             ;
                    UnBoostedDaughters [2] = Daughters [2].LorentzBoostGamma (Velocity)             ;
                    UnBoostedTop           = Top.LorentzBoostGamma           (Velocity)             ;
                }
                /* Pre Evaluate some important variables for efficiency: */ {
                    e2                     = UnBoostedDaughters[0][3]                               ;
                    /* Debugging check: */ { goodvals = (e2>0)                    ; if (!goodvals) {printf("BAD: e2<0\n");return goodvals;} }
                    mt                     = UnBoostedTop.m()                                       ;
                    mt2                    = mt * mt                                                ;
                    mte2                   = mt * UnBoostedDaughters[0][3]                          ;
                    st2                    = UnBoostedDaughters[0].pt() / UnBoostedDaughters[0].p() ;
                    /* Debugging check: */ { goodvals = (st2>=-1.0) && (1.0>=st2) ; if (!goodvals) {printf("BAD: |st2|>1\n");return goodvals;} }
                    dir2p                  = UnBoostedDaughters[0].xyz.dir()                        ;
                }
                return prepare2 () ;
            }
            inline bool EvalMeasure () {
                double num = mte2*((2.0*UnBoostedDaughters[0][3])-mt)*st2*st3 ;
                double den = ((UnBoostedDaughters[0][3]*(1.0-angle))-mt)      ;
                den        = den * den * 8.0                                  ;
                measure    = CPPFileIO::mymod (num/den)                       ;
                goodvals   = (measure>=0)                                     ;
                if (!goodvals) {printf("BIG FAT ALARM: measure < 0...\n");return goodvals;}
                return goodvals                                               ;
            }
            inline bool PrepMatrix  () {
                double num = g4 * (mt-(2.0*e3)) * e3 ; /* The Debugging part: */ {
                    goodvals = (num>0) ;
                    if (!goodvals)  {return goodvals;}
                    //if (!goodvals)  {printf("BIG FAT ALARM: matrix numerator < 0...\n");return goodvals;}
                }
                double finalden ; /* This is a complex number because of W width: */ {
                    double den[2] ;
                    /* The real part: */ {
                        den[0] = NewHEPHeaders::MASS::W / mt ;
                        den[0] = den[0] * den[0] ;
                        den[0] = den[0] + (2.0*e2/mt) ;
                        den[0] = 1.0 - den[0] ;
                        den[0] = den[0] * den[0] ;
                    }
                    /* The imaginary part: */ {
                        den[1] = gammaw * NewHEPHeaders::MASS::W / mt2 ;
                        den[1] = den[1] * den[1] ;
                    }
                    finalden = den[0] + den[1] ; /* The Debugging part: */ {
                        goodvals = (finalden>0) ;
                        if (!goodvals)  {printf("BIG FAT ALARM: matrix denominator < 0...\n");return goodvals;}
                    }
                }
                matrix2  = num / finalden ; /* The debugging part: */ {
                    goodvals = (matrix2>=0)   ;
                    if (!goodvals)  {printf("BIG FAT ALARM: matrix < 0...\n");return goodvals;}
                }
                //printf("Good: Matrix good\n");
                return goodvals;
            }
            inline bool FinalWeight () {
                weight = (matrix2*measure) / (total_top_gamma*mt2) ;
                goodvals = (weight>0) ;
                //if (!goodvals) {printf("BIG FAT ALARM: Final Weight < 0...\n");return goodvals;}
                if (!goodvals) {return goodvals;}
                return goodvals ;
            }
            inline double EvalMatrix ( vector3 Spin ) { return weight * (1.0+(Spin*dir3p)) ; }
            inline void SwapVectors () { CPPFileIO::myswap (UnBoostedDaughters[1],UnBoostedDaughters[2]) ; }

        public:
            double FinalWeights[2] ;
            int    FinalRecord     ;
            double dot             ;
            TopTester ( vector4 a , vector4 b , vector4 c ) {
                dot             = -10000 ;
                FinalWeights[0] = -10000 ;
                FinalWeights[1] = -10000 ;
                FinalRecord     = -1     ;
                goodvals        = true   ;
                if (prepare(a,b,c)) if (EvalMeasure()) if (PrepMatrix()) if (FinalWeight()) {
                    FinalWeights[0] = weight                                          ;
                    dot             = (direction.xyz*UnBoostedDaughters[1].xyz.dir()) ;
                }
                SwapVectors ()           ;
                goodvals        = true   ;
                if (prepare2()) if (EvalMeasure()) if (PrepMatrix()) if (FinalWeight()) {FinalWeights[1]=weight;}
                if (FinalWeights[0]>=0) {
                    if      ( FinalWeights[0] >= FinalWeights[1] ) {FinalRecord=3;}
                    else if ( FinalWeights[1] >  FinalWeights[0] ) {FinalRecord=4;}
                } else if   ( FinalWeights[1] >= 0               ) {FinalRecord=4;}
                if (FinalRecord==4) { dot = (direction.xyz*UnBoostedDaughters[1].xyz.dir()) ; }
            }
            ~TopTester(){}
        };
        class reco_top_had {
        ...
        public:
            ...
            inline int GetMatrixIndex () {
                vector4 p[3] ; /* Get pt ordered jets: */ {
                    p[0] = bjet[0]    ;
                    p[1] = njet[0][0] ;
                    p[2] = njet[1][0] ;
                }
                if ( p[2].pt() > p[1].pt() ) { CPPFileIO::myswap (p[1],p[2]) ; }
                TopTester tmp (p[0],p[1],p[2]) ;
                return tmp.FinalRecord ;
            }
            inline double GetMatrixDot () {
                vector4 p[3] ; /* Get pt ordered jets: */ {
                    p[0] = bjet[0]    ;
                    p[1] = njet[0][0] ;
                    p[2] = njet[1][0] ;
