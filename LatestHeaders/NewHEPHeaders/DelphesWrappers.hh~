		...
		operator () (
			...
			if (this->fChain != 0) {
				...
				for (
					...
					long ientry =
						this->LoadTree(jentry)
					; //
					if (ientry >= 0) {
						nb =
							this->fChain->GetEntry
								(jentry)
						; //
						if (nb>0) {
							nbytes += nb;
				...
				for (
					...
					TYPE_Element tmp2 ; /* Read in the track vectors: */ {
						tmp2.SetPtEtaPhiM (
							...
							if (
								CPPFileIO::mymod (inref.EFlowTrack_PID[i]) ==
									PID::MUON
							) {
								tmp2.Eem  = tmp2[3] * 0.0 ;
